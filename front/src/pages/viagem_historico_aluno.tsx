...
const ViagemHistorico = () => {
  ...
  // Abrir modal Ver
  const handleVer = async (viagem: Viagem) => {
    setViagemSelecionada(viagem);
    setShowModalVer(true);

    try {
      const dadosValidacoes = await validacaoService.getValidacoesAlunoViagem(viagem.ID_viagem!, usuario!.sub);
      setValidacoes(dadosValidacoes);
      console.log('Validações carregadas:', dadosValidacoes);
    } catch (error) {
      console.error("Erro ao buscar validações:");
      setMensagem("Não foi possível carregar as validações da viagem.");
      setTipoMensagem("error");
    }
  };
  ...
  if (usuario.perfil != 3) return <p>É preciso ser Administrador para acessar essa página</p>;
          ...
          <Modal.Body>
            ...
            <Card className="shadow-sm">
              <Card.Header as="h5">
                ...
                Minhas Validações na Viagem
              </Card.Header>
              <Card.Body>
                {validacoes.length === 0 ? (
                  <p className="text-muted mb-0">Nenhuma validação encontrada para você nesta viagem.</p>
                ) : (
                  <Table striped bordered hover responsive size="sm" className="mb-0">
                    ...
                    <tbody>
                      {validacoes.map((validacao, index) => (
                        <tr key={index}>
                          <td>
                            <span className={`badge fs-6 rounded-pill text-capitalize bg-${validacao.tipo_evento === 'embarque' ? 'success' : 'danger'}`}>
                              {validacao.tipo_evento}
                            </span>
                          </td>
                          <td>{validacao.parada?.nome ?? 'Parada não informada'}</td>
...
export default ViagemHistorico;
