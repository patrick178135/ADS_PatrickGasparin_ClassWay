...
export class ValidacaoController {
  ...
  @Get('viagem/:idViagem/aluno/:idAluno')
  async findValidacoesAlunoViagem(
    @Param('idViagem') idViagem: number,
    @Param('idAluno') idAluno: number,
  ) {
    const idViagemNum = +idViagem;
    const idAlunoNum = +idAluno;
    
    return this.validacaoService.getValidacoesAlunoViagem(idViagemNum, idAlunoNum);
  }
